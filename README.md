...
This is a **learning platform, not a product**. A real team monitoring eight services would run Grafana, or consolidate onto one or two datastores. Fleet Monitor deliberately spreads its data across five stores — Postgres, Redis, ClickHouse, InfluxDB, Neo4j — chosen one job per store, so that each choice can be *measured* against the alternative instead of just asserted. Defending this as a production architecture would be the wrong move; the five [comparison write-ups](docs/comparisons/) are the actual point.

Three things it's built to teach, in priority order:

1. **Storage selection** — build the same feature on Postgres first, hit its limits, move to a specialized store, record the difference.
2. **Ingest engineering** — buffering, batching, and backpressure at real write volume, not toy volume.
3. **Agent engineering** — a tool-using agent with real guardrails, evaluated against known ground truth rather than demoed against a happy path.
